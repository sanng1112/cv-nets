"""Tests for DiceLoss."""

from __future__ import annotations

import torch
import pytest
from cvnets.loss_fn import build_loss_fn, SUPPORTED_LOSSES


class TestDiceLoss:
    """Test suite for DiceLoss."""

    def test_registered(self):
        """DiceLoss is registered in SUPPORTED_LOSSES."""
        assert "segmentation/dice_loss" in SUPPORTED_LOSSES

    def test_build(self):
        """Can build via build_loss_fn."""
        fn = build_loss_fn("dice_loss", category="segmentation")
        assert fn is not None

    def test_basic(self):
        """Forward returns a scalar in [0, 1] range."""
        fn = build_loss_fn("dice_loss", category="segmentation")
        out = fn(torch.randn(4, 5, 32, 32), torch.randint(0, 5, (4, 32, 32)))
        assert out.shape == ()
        assert 0.0 <= out.item() <= 1.0

    def test_perfect_zero(self):
        """Perfect prediction yields near-zero loss."""
        fn = build_loss_fn("dice_loss", category="segmentation")
        p = torch.full((2, 3, 8, 8), -100.0)
        p[:, 0, :, :] = 100.0
        loss = fn(p, torch.zeros(2, 8, 8, dtype=torch.long))
        assert loss.item() < 0.05

    def test_binary(self):
        """Binary mode works with sigmoid input."""
        fn = build_loss_fn("dice_loss", category="segmentation", binary=True)
        p = torch.sigmoid(torch.randn(4, 1, 16, 16))
        out = fn(p, torch.randint(0, 2, (4, 1, 16, 16)).float())
        assert out.shape == ()

    def test_forward_sum_reduction(self):
        """Sum reduction works."""
        fn = build_loss_fn("dice_loss", category="segmentation", reduction="sum")
        out = fn(torch.randn(4, 5, 32, 32), torch.randint(0, 5, (4, 32, 32)))
        assert out.shape == ()

    def test_forward_none_reduction(self):
        """None reduction returns per-batch losses."""
        fn = build_loss_fn("dice_loss", category="segmentation", reduction="none")
        out = fn(torch.randn(4, 5, 32, 32), torch.randint(0, 5, (4, 32, 32)))
        assert out.shape == (4,)

    def test_extra_repr(self):
        """extra_repr includes key parameters."""
        fn = build_loss_fn("dice_loss", category="segmentation", smooth=1e-5, binary=True)
        r = fn.extra_repr()
        assert "smooth" in r
        assert "binary" in r
