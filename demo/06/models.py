...
from blocks.ResnetBlock import ResNetBasicBlock
...
class CNN(nn.Module):
    def __init__(
        ...
        for cfg in layers_cfg:
            ...
            # --- KHỐI RESNET (MỚI THÊM) ---
            elif layer_type == "resnetbasicblock":
                in_ch = getattr(cfg, "in_channels", None)
                out_ch = getattr(cfg, "out_channels", None)
                stride = getattr(cfg, "stride", 1)
                
                # Gọi ResNetBasicBlock (đã lấy in/out channels từ YAML)
            ...
            # --- KHỐI PHÂN LOẠI (CLASSIFIER) ---
                ...
                # Tự động thêm Flatten trước lớp FC đầu tiên
                ...
                pool_layer = build_pooling_layer(opts=cfg)
        ...
        if train_cfg and hasattr(train_cfg, "dropout"):
            ...
            if len(classifier_layers) > 1:
                # Chèn Dropout trước lớp FC cuối cùng
