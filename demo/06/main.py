import yaml
from types import SimpleNamespace
from tqdm import tqdm
import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader
from torchvision import datasets, transforms
from models import * 

def dict_to_namespace(d):
    if isinstance(d, dict):
        return SimpleNamespace(**{
            k: dict_to_namespace(v)
            for k, v in d.items()
        })
    elif isinstance(d, list):  
        return [dict_to_namespace(v) for v in d]
    return d

CONFIG = 'config.yaml'

with open(CONFIG, "r") as f:
    cfg = yaml.safe_load(f)

opts = dict_to_namespace(cfg)
print(opts)

model = CNN(opts=opts)
print(model)

class AddGaussianNoise(object):
    def __init__(self, mean=0.0, std=0.15):
        self.std = std
        self.mean = mean
    def __call__(self, tensor):
        noise = torch.randn_like(tensor) * self.std + self.mean
        return torch.clamp(tensor + noise, 0.0, 1.0)

# ==========================================
# 1. TÁCH BIỆT TRANSFORM CHO TRAIN VÀ TEST
# ==========================================

# Transform Tập Train (Làm giàu dữ liệu mạnh tay)
train_transform = transforms.Compose([
    transforms.Grayscale(num_output_channels=1),
    transforms.RandomRotation(degrees=15),                               # Xoay ảnh mô phỏng nghiêng đầu
    transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.9, 1.1)),
    transforms.RandomHorizontalFlip(p=0.5),                              # Lật ngang (rất quan trọng với khuôn mặt)
    transforms.ColorJitter(brightness=0.2, contrast=0.2),                # Đổi độ sáng tối
    transforms.ToTensor(),
    transforms.Normalize((0.5,), (0.5,)), 
    transforms.RandomErasing(p=0.2, scale=(0.02, 0.1), value=0),         # Mô phỏng che khuất (Occlusion)
    transforms.RandomApply([AddGaussianNoise(0.0, 0.05)], p=0.5) 
])

# Transform Tập Test (TUYỆT ĐỐI KHÔNG làm méo/nhiễu ảnh)
test_transform = transforms.Compose([
    transforms.Grayscale(num_output_channels=1),
    transforms.ToTensor(),
    transforms.Normalize((0.5,), (0.5,))
])

train_dataset = datasets.ImageFolder(
    root='./data/train', 
    transform=train_transform
)
test_dataset = datasets.ImageFolder(
    root='./data/test', 
    transform=test_transform   # <-- SỬA LỖI TẠI ĐÂY
)

def update_dataloader(dataset, current_batch_size, scale=1.2):
    """Tính toán batch size mới và khởi tạo lại DataLoader"""
    new_batch_size = int(current_batch_size * scale)
    print(f"\n🚀 [CẬP NHẬT] Batch size tăng từ {current_batch_size} lên {new_batch_size}")
    
    new_loader = DataLoader(
        dataset,
        batch_size=new_batch_size,
        num_workers=16, 
        shuffle=True,
        pin_memory=True
    )
    return new_loader, new_batch_size

current_batch_size = 128
train_loader = DataLoader(
    train_dataset,
    batch_size=current_batch_size,
    num_workers=16, 
    shuffle=True,
    pin_memory=True
)

test_loader = DataLoader(
    test_dataset,
    batch_size=64,
    num_workers=8, 
)

data_iter = iter(train_loader)
images, labels = next(data_iter)
print(f"Kích thước tensor ảnh: {images.shape}")
print(f"Số lượng nhãn: {len(train_dataset.classes)}")

# ==========================================
# 2. LABEL SMOOTHING & COSINE SCHEDULER
# ==========================================

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model.to(device)

# Áp dụng Label Smoothing = 0.1 (Chống Overconfidence)
criterion = nn.CrossEntropyLoss(label_smoothing=0.1).to(device)

num_epochs = 100
optimizer = optim.Adam(model.parameters(), lr=5e-3)

# Áp dụng Cosine Annealing (Tự động uốn nắn LR mượt mà như hình sin)
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=1e-5)

best_val_acc = None 

for epoch in range(num_epochs):
    model.train()
    running_loss = 0.0
    correct = 0  
    total = 0    
    
    # In ra Learning Rate hiện tại của epoch
    current_lr = optimizer.param_groups[0]['lr']
    loop = tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs} [LR: {current_lr:.6f}]")
    
    for images, labels in loop:
        images, labels = images.to(device), labels.to(device)
        optimizer.zero_grad()
        
        outputs = model(images)
        loss = criterion(outputs, labels)
        
        loss.backward()
        optimizer.step()
        
        _, predicted = torch.max(outputs.data, 1)
        total += labels.size(0)
        correct += (predicted == labels).sum().item()
        
        running_loss += loss.item() * images.size(0)
        loop.set_postfix(loss=loss.item())

    # Cập nhật LR bằng Scheduler ở cuối mỗi epoch
    scheduler.step()

    # --- ĐÁNH GIÁ (EVAL) ---
    model.eval()
    val_loss = 0.0
    val_correct = 0
    val_total = 0
    
    val_loop = tqdm(test_loader, desc=f"Epoch {epoch+1}/{num_epochs} [TEST]")
    
    with torch.no_grad():
        for images, labels in val_loop:
            images, labels = images.to(device), labels.to(device)
            
            outputs = model(images)
            loss = criterion(outputs, labels)
            
            val_loss += loss.item() * images.size(0)
            _, predicted = torch.max(outputs, 1)
            val_total += labels.size(0)
            val_correct += (predicted == labels).sum().item()
            
            val_loop.set_postfix(loss=loss.item(), acc=100.*val_correct/val_total)

    epoch_val_loss = val_loss / len(test_dataset)
    epoch_val_acc = 100. * val_correct / val_total

    print(f"\nSummary Epoch {epoch+1}:")
    if best_val_acc is not None:
        print(f"Val Loss: {epoch_val_loss:.4f} | Val Acc: {epoch_val_acc:.2f}% (Mốc hiện tại: {best_val_acc:.2f}%)")
    else:
        print(f"Val Loss: {epoch_val_loss:.4f} | Val Acc: {epoch_val_acc:.2f}%")
    print("-" * 40)

    if best_val_acc is None:
        best_val_acc = epoch_val_acc
    elif epoch_val_acc >= best_val_acc + 10.0:
        print(f"🔥 Tuyệt vời! Val Acc tăng vượt 10% (Từ {best_val_acc:.2f}% lên {epoch_val_acc:.2f}%)")
        best_val_acc = epoch_val_acc
        
        # Chỉ giữ lại logic tăng Batch Size. Bỏ hàm giảm LR tay vì Scheduler đã làm tốt hơn.
        train_loader, current_batch_size = update_dataloader(
            train_dataset, current_batch_size, scale=1.2
        )

# Chỉ lưu một lần sau khi huấn luyện xong (tránh ghi đè file liên tục gây chậm I/O)
model.save()
print("\nHoàn tất huấn luyện và đã lưu mô hình!")