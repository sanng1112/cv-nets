...
@LOSS_REGISTRY.register(name="tversky", type="segmentation")
class TverskyLoss(BaseSegmentationCriteria):
    def __init__(self, opts: argparse.Namespace, *args, **kwargs) -> None:
        ...
        self.smooth = getattr(opts, "loss.segmentation.tversky.smooth", 1.0)
        self.alpha = getattr(opts, "loss.segmentation.tversky.alpha", 0.5)
        self.beta = getattr(opts, "loss.segmentation.tversky.beta", 0.5)
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        group.add_argument(
            "--loss.segmentation.tversky.smooth",
            type=float,
            default=1.0,
            help="Smoothing factor for Tversky Loss. Defaults to 1.0.",
        )
        group.add_argument(
            "--loss.segmentation.tversky.alpha",
            type=float,
            default=0.5,
            help="Alpha for False Positives penalty. Defaults to 0.5.",
        )
        group.add_argument(
            "--loss.segmentation.tversky.beta",
            type=float,
            default=0.5,
