        ...
        **kwargs,
        ...
        if isinstance(prediction, Tensor):
            ...
            if "boxes" not in prediction:
                logger.error(f"boxes is a required key in {self.__class__.__name__} when prediction type is dictionary.")
            return self._compute_loss(prediction=prediction["boxes"], target=target, *args, **kwargs)
        else:
