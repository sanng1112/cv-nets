...
@LOSS_REGISTRY.register(name="__base__", type="regression")
class BaseRegressionCriteria(BaseCriteria):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        - Cảnh báo: Tham số đầu vào cần tuân thủ cấu trúc chuẩn của module.
        """
        if cls != BaseRegressionCriteria:
            return parser

        group = parser.add_argument_group(cls.__name__)
        group.add_argument(
            "--loss.regression.name",
            type=str,
            default=None,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(prediction, Tensor):
            ...
            if "preds" not in prediction:
                logger.error(f"'preds' is a required key in {self.__class__.__name__} when prediction type is dictionary.")
            return self._compute_loss(prediction=prediction["preds"], target=target, *args, **kwargs)
        else:
