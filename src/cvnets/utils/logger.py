"""
Logger module for cv-nets.

Provides color-coded logging with stdout/stderr control.

Critical difference from the original ``utils/logger.py``:
    - ``error()`` raises ``LoggerError`` instead of calling ``sys.exit()``.
...
class LoggerError(Exception):
    """Raised by ``error()`` instead of calling ``sys.exit()``."""
...
def error(message: str) -> None:
    ...
    raise LoggerError(
        "{} - {} - {}. Exiting!!!".format(time_stamp, error_str, message)
    )
